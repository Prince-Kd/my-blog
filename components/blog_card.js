  ...
  return (
    <div className="lg:h-52 lg:grid lg:grid-cols-4 mb-10 shadow-md lg:p-4 p-2 flex justify-between">
      <div className="lg:col-span-3">
        <div id="title" className="lg:text-xl text-lg lg:font-bold font-semibold">{post.title.toUpperCase()}</div>
        <div id="author" className="flex flex-row items-center">
            ...
            <div className="lg:h-6 lg:w-6 h-5 w-5 rounded-full bg-gray-200">
                <img src={post.authorPhoto} alt="dp" className="lg:h-6 lg:w-6 h-5 w-5  rounded-full flex items-center justify-center text-xs" />
            </div>
        </div><br/>
        {/* <div id="summary" className=" flex-wrap font-medium">
          {post.post}
        </div><br/> */}
        <div id="details" className="flex lg:flex-row flex-col justify-between lg:items-center lg:text-sm text-xs text-gray-500">
      ...
      <div id="image" className=" lg:h-48 lg:w-52 h-24 w-24 bg-gray-200 ">
          <img className=" lg:h-48 lg:w-52 h-24 w-24" src={post.postImage} alt="poster" />
      </div>
    </div>
  );
}
